...
if {$opt(reset)} {
  open_project -reset DC_prj
} else {
  open_project DC_prj
}
set_top DC
add_files firmware/DC.cpp -cflags "-std=c++0x"
add_files -tb DC_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb DC_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
...
if {$opt(vsynth)} {
  ...
  if {[file exist DC_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
